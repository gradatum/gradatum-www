...
const versions: VersionPhase[] = [
  ...
  },
  {
    ...
    featureRefs: ['F-18', 'F-45', 'F-131', 'F-51', 'F-70', 'F-111'],
    showFeaturesLink: false,
  },
  {
    version: 'v2.0.0',
  ...
  },
  {
    version: 'v2.5.0',
    status: 'done',
    theme: 'Privacy Sidecar, Fail-Closed ACL & Queue Erasure',
    description:
      'v2.5.0 ships the whole personal-data detection frontier as a complete, inert unit: gradatum-privacy gains a confined out-of-process sidecar detector reached over a unix socket, the masking mode that rewrites every passage it marks, and the [privacy] wiring that turns thresholds, categories and exceptions into a decision — none of it marks, rewrites or refuses anything until an operator activates a category, which stays deferred to 2.6.0. Masking mode is opt-in and destructive: switching it on purges the version history of every note it rewrites, and a one-shot history-purge command reaches what an earlier switch left behind. Two ACL gates close a fail-open gap — a preset that is present but cannot be loaded now refuses to start the server instead of falling back to deny-all in silence, and acl validate names every consumer whose read patterns hold no positive pattern. The job queue\'s terminal payloads are now erased physically, in bounded batches, on a schedule, and the release gate\'s semver check now asserts allowed behaviour per version rank instead of two hard-coded anchor entries. The 2.4.0 → 2.5.0 upgrade guide documents every one of these changes for an operator moving a live deployment.',
    scopeTeaserItems: [
      'Out-of-process privacy sidecar (gradatum-privacy) ships complete and inert — categories empty, activation deferred to 2.6.0',
      'Masking mode is opt-in and destructive — rewrites marked spans and purges the note\'s version history; history-purge reaches what came before',
      'A configured-but-unusable ACL preset now refuses to start the server; acl validate names every consumer that reads nothing',
      'Job queue terminal payloads erased physically (secure_delete=ON), in bounded batches, on a schedule',
      'Release gate\'s semver check asserts behaviour per version rank; the 2.4.0 → 2.5.0 upgrade guide is published',
    ],
    featureRefs: [],
    showFeaturesLink: false,
    milestones: [
      {
        status: 'done',
        title: 'The personal-data detection frontier ships complete and inert — sidecar detector, masking mode and [privacy] wiring all present, no category activated by default',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'A sidecar detector for personal-data recognition runs out of process behind a unix socket, confined and supervised, and a call that does not complete is never reported as a scan that found nothing',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'Masking mode rewrites every marked span and is refused at load unless at least one activated category sits above a zero threshold',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'A masking write destroys the note\'s whole version history, and privacy history-purge reaches the snapshots an earlier switch to mask left behind',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'An ACL preset that is present but cannot be loaded now refuses to start the server, instead of falling back to deny-all in silence',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'acl validate names every consumer whose read patterns hold no positive pattern — a preset that reads nothing now says so',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'The job queue\'s terminal payloads are erased physically, in bounded batches, on a schedule, instead of only logically',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'The release gate\'s semver check now asserts behaviour per version rank, closing the last hard-coded anchor entries',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
      {
        status: 'done',
        title: 'Tagged — 28 crates published, 5,002 tests PASS',
        version: 'v2.5.0',
        date: '2026-09-17',
      },
    ],
  },
  {
    version: 'v2.6.0',
    status: 'planned',
    theme: 'Personal-Data Category Activation',
    description:
      'v2.6.0 is planned to turn the personal-data marking pass shipped inert in 2.5.0 into an active one: a measurement campaign informs which categories are safe to turn on, category by category, rather than activating on an uncalibrated threshold. The CI build-directory observation windows opened alongside the 2.5.0 garbage-collection work are expected to close on a normal-regime measurement, not on more code.',
    scopeTeaserItems: [
      'Category-by-category activation of the personal-data marking pass, informed by a measurement campaign',
      'CI build-directory observation windows close on measurement, not on more work',
      'Audit debt items carried forward from the 2.4.0 / 2.5.0 cycle',
    ],
    featureRefs: [],
    showFeaturesLink: false,
  },
